...
public class HodServiceImpl implements HodService {
    ...
    // Daily
@Override
public Map<String, List<AttendanceReportDTO>> getDailyReport() {

    List<Attendance> list = attendanceRepository.findDailyReport();

    return list.stream()
            .collect(Collectors.groupingBy(
                    a -> a.getSession().getClassEmail().substring(0,
                            a.getSession().getClassEmail().indexOf("@")),
 ...
 @Override
public Map<String, List<AttendanceReportDTO>> getMonthlyReport() {

    LocalDateTime oneMonthAgo = LocalDateTime.now().minusMonths(1);

    List<Attendance> list = attendanceRepository.findReportAfter(oneMonthAgo);

    return list.stream()
            .collect(Collectors.groupingBy(
                    a -> a.getSession().getClassEmail()
                            .substring(0, a.getSession().getClassEmail().indexOf("@")),
                    Collectors.mapping(this::mapToDTO, Collectors.toList())
            ));
}
@Override
public Map<String, List<AttendanceReportDTO>> getWeeklyReport() {

    LocalDateTime oneWeekAgo = LocalDateTime.now().minusDays(7);

    List<Attendance> list = attendanceRepository.findReportAfter(oneWeekAgo);
